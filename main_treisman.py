...
from src.composer import Alphabets, Treisman
...
train_params = {
    ...
    "lr": argus.lr,
    "l2": argus.l2,
    "exase": argus.exase+"_"+str(argus.mode),
    "dir": r"./results",
    "milestones": [32, ],
    "gamma": 0.2,
    "max_grad_norm": 1.0,
...
model_params = {
    ...
    "rnn_dims": (32, 16),  # dimensions of the RNN (First value is not RNN but FC)
...
tasks = OrderedDict({})
tasks["Treisman"] = {
    "composer": Treisman,  # composer (torch Dataset)
    "key": 0,  # key for the task
    "params": {
        "in_dims": model_params["in_dims"], 
        "n_iter": argus.n_iter, 
        "spurious_ratio": 0.99 if argus.mode == 0 else 0.01 if argus.mode == 1 else 1.0 if argus.mode == 2 else 0.9,
        "digit_intensity": 1.0,
        "noise": 0.125,
        "spurious_background": None,
        "spurious_class": None,
        "spurious_alpha": False if argus.mode == 3 else True if argus.mode == 4 else False,
        },
    "datasets": [],
    "dataloaders": [],
...
train_alpha = Alphabets(directory=data_path, kind="train")
valid_alpha = Alphabets(directory=data_path, kind="valid")
test_alpha = Alphabets(directory=data_path, kind="test")
DeVice, num_workers, pin_memory = get_device()
for o in tasks:
    tasks[o]["datasets"].append(tasks[o]["composer"](train_alpha, **tasks[o]["params"], kind="train", n_samples=1024*16))
    tasks[o]["datasets"].append(tasks[o]["composer"](valid_alpha, **tasks[o]["params"], kind="valid", n_samples=-1))
    tasks[o]["datasets"].append(tasks[o]["composer"](test_alpha, **tasks[o]["params"], kind="test", n_samples=-1))
