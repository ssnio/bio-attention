...
model_params = {
    ...
    "channels": (3, 4, 8, 16, 32, 64, 64),  # channels in the encoder
    ...
    "conv_funs": torch.nn.Tanh(),  # activation function in the encoder
    "deconv_funs": torch.nn.Tanh(),  # activation function in the decoder
    "deconv_norms": None,  # normalization in the decoder
    "pools": (1, 2, 2, 2, 2, 2),  # pooling in the encoder
    "rnn_dims": (32, ),  # dimensions of the RNN (First value is not RNN but FC)
    "rnn_bias": True,  # bias in the RNN
    "rnn_dropouts": 0.0,  # dropout in the RNN
    "rnn_funs": torch.nn.Tanh(),  # activation function in the RNN
...
if argus.mode == 0:
    spurious_ratio, spurious_self, outline_ratio, ambiguous = 0.8, False, 0.0, True
...
for o in tasks:
    tasks[o]["datasets"].append(tasks[o]["composer"](**tasks[o]["params"], kind="train", n_samples=16*1024))
    tasks[o]["datasets"].append(tasks[o]["composer"](**tasks[o]["params"], kind="valid", n_samples=256))
    tasks[o]["datasets"].append(tasks[o]["composer"](**tasks[o]["params"], kind="test", n_samples=256))
...
conductor.train(train_params["n_epochs"], DeVice, False, mask_mp=train_params["lm"])
